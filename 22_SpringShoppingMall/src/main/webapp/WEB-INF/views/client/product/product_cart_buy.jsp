...
<script>
	...
	function buy_complete(){
		var buy_complete_idx =  []
		var buy_complete_product_idx =  []
		var buy_complete_cnt = []
		var buy_complete_total_price = []
		var buy_complete_price = []
		
		<c:forEach var="obj" items="${requestScope.cart_list}">
		
			buy_complete_idx.push(${obj.cart_idx})
			
			buy_complete_product_idx.push(${obj.cart_product_idx})
			
			buy_complete_cnt.push(${obj.cart_cnt})
			
			buy_complete_total_price.push(${obj.cart_total_price})
			
			buy_complete_price.push(${obj.cart_price })
			
		</c:forEach>
			
		var params = {
				buy_complete_idx : buy_complete_idx,
				buy_complete_product_idx : buy_complete_product_idx,
				buy_complete_cnt :buy_complete_cnt,
				buy_complete_total_price : buy_complete_total_price,
				buy_complete_price : buy_complete_price
		}

		$.ajax({
			url : 'cart_complete_pro.springshoppingmall',
			type : 'post',
			data : params,
			traditional : true,
			dataType : 'text',
...
<div class='container' style='margin-top:50px'>
	...
	<div class='row center'>
		...
		<div class='card shadow col-md-5' style ='margin-left:20px'>
			...
			<div class='display-3'><fmt:formatNumber value="${cart_buy_sum }" type="number"/>원</div>
			
			<h4 class='text-danger'>결제시 환불은 여행 당일 3일전 까지 <br/> 100% 환불가능 합니다.</h4>
			<br/>
			<br/>
			<div class ='container text-right'style ='margin-top:50px'>
				<button type ='button' class='btn btn-primary' onclick ="buy_complete()"style='width:250px;'>구매확정</button>
				<a href='buy_cart.springshoppingmall?page_num=${page_num }' class='btn btn-danger' style='width:250px;margin-top:25px'>취소하기</a>
			</div>
		</div>
	</div>
</div>
